...
AHBA_degeprimer <- degeprimer(AHBAseqs) %>%
  run_alignment() %>%
  build_tree() %>%
  design_primers(degeneracyrange=c(1, 10, 20), number_iterations=2) # this can take awhile
```
...
Interactive inspection of primer pairs is used to select primers. users can use that information and design their own primers or they can explore the coverage of their primers across their target sequences. 
...
Once primers are chosen it is desireable to view coverage of the target sequences. The *plot_coveragematrix* will calcualte expected coverage of each primerpair in the *degeprimer* object by serahing for exact matches of the forward and reverse primers against your target sequences. The most important parameter, is the max.mismatch which determines how many nucleotide differences are allowable between the degernate primer and the target sequence.
...
While many degenerate priemr design projects are begun with sequences in mind, in many cases involving metagenomes, you may wish to design primers to maximize coverage to a target with only a few initial sequences. DegeneratePrimerTools provides helper functionts to retrieve DNA sequences corresponding to the conserved PFAM domain protein sequences. To retrieve all DNA sequences that encode the PFAM16997 domain, for example, you could do the following:
...
3. maps the UNIPROT protein accessions to ENA nuceleotide accessions
4. retrieves the ENA nucleotide accessions.
5. Extracts the regions of the DNA sequences corresponding to the protein domain.

Because of this workflow the default PFAM alignment type that is retrieved is the "uniprot" alignment corresponding to all matches of the PFAM HMM against the full UNIPROT reference set. All of the publically avaialbel options - "seed", rp15", "meta" etc - are supported but in the case of large alignments (eg. the "NCBI" alignments), the PFAM website may not serve these files. In this case it will be necessary to downlaod the full dataset from the PFAM FTP site and parse these accessions directly.

As a workflow for the retieval you could use a variation of the following:
...
DegeneratePrimerTools uses several classes that encpasulate data. These are the following:
...
2. The *primerpair* class allows you define forward and reverse prierm pairs.

3. The *degeprimer* class.  This is the class holding slots for nucleic acid sequence data, multiple sequence alignments, phlogenetic trees, *primerdata* DEGEPRIME tabular output and a list of *priemrpairs*.

By providing a single object that contains all of the data, we can provide astandard interface for processing ata via pipes as well as provide a single target for visualization functions.
